---
name: zk-ingest
description: Ingest a raw source file through the full zk-wiki pipeline (raw → zettel extraction → wiki update → index/log). Usage: /zk-ingest <file-path-or-glob>
...
3. **ingester agent**에 위임:
   - 아래 정보를 전달:
     ...
     - 적용 skill: ingest-pipeline, zettelkasten-conventions, wiki-maintenance
   - Agent를 호출하여 7단계 파이프라인 전체 실행
